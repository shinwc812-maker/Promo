...
> - `COUPON_COUNTS` = 쿠폰 evntNo → 발행수(정수, 명시된 경우만)
...
### 6.5단계: 쿠폰 발행수 → COUPON_COUNTS
쿠폰 이벤트 이미지에 **"총 N장"·"선착순 N명"** 처럼 수량이 명시된 경우만 수집.
- "선착순 300명" → 300명 전원 수령 가정 → **300** (장)
- 그냥 "선착순 다운로드"·수량 없음 → dict 에 안 넣음 → 미공개
- 쿠폰은 영화관이 발행수를 거의 공개 안 해 대부분 미공개지만, **있으면 꼭 수집**
```python
COUPON_COUNTS = {
    # "202605XXXXXX": 300,   # 예: 선착순 300명
}
```
