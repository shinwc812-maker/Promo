...
def classify(name, class_code):
    ...
    if any(k in name for k in GOODS_KW):
        return "goods"
    # '상영회'는 '프리미어'와 함께 있을 때만 시사회(stage). 그 외는 기타.
    # (롯데 분류코드 40 무대인사/시사회는 위에서 이미 stage 로 분기됨)
    if "프리미어" in name and "상영회" in name:
        return "stage"
    return "etc"
...
def main():
    ...
    # 종료일(상영일/사용기한/지급기한) 지난 이벤트는 타입 무관 모두 제외.
    ...
    for ev in events:
        ...
        if end and end < today:            # 종료일 지난 이벤트는 타입 무관 제외
            continue
    ...
    # (롯데는 좌석 합산이 LCWS API 자동 — SCREENINGS dict 후행 보강 불필요)
